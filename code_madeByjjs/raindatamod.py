import pandas as pd
#지역별 월 강수량 데이터 생성
# 현재 data\2022 서울 지역구별 월 강수량 by pandas에 있는 파일을 만드는 프로그램입니다.
'''
...
for mon in ["01","02","03","04","05","06","07","08","09","10","11","12"]:    
    ...
    for v in range(len(dailyRain)):
        if(monthlyRain[v] == 0):
            ...
            if(dailyRain[v] != 0):
                ...
                value.append(-1) # monthlyRain 값을 넣어 주기 위해 -1 입력 
    # 데이터 출력
    #print(value)
    '''print(pd.DataFrame({
        '강수량' : dailyRain,
    ...
    for dist in distName:
        ...
        amount.append(temp['10분우량'].sum())
    data = {
        '구청명' : distName,
        '월강수량' : amount
    }
    rainData = pd.DataFrame(data)
    rainData.to_csv(f'./data/2022 서울 지역구별 월 강수량 by pandas/서울 {mon}월 지역구별 강수량.csv',encoding='utf-8-sig')
